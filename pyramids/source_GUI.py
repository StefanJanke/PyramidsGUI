...
class Image():
    ...
    def __init__(
        ...
        pixel_size_units='µm',
        autocrop=True, 
        bool_small_pyramids = False):
        if file_name:
            self.load(file_name, True)
        else:
            self.data = np.zeros([32, 32])
        self.set_scale(pixel_size, pixel_size_units)
        if autocrop:  # Remove the bottom info strip of the SEM image
            self.data = self.data[:750, :]
        if bool_small_pyramids:
            self.data = self.data[:375, :375]
    ...
    def show2(self, scalebar_size=None):
        fig = plt.figure(figsize=(20,8), dpi = 60)
        ax1 = fig.add_subplot(1,2,1)
        ax1.imshow(self.data, cmap=plt.cm.gist_gray)
        plt.axis('off')
        ax2 = fig.add_subplot(1,2,2)
        ax2.imshow(self.data[:750, :], cmap=plt.cm.gist_gray)
        plt.axis('off')
        plt.tight_layout(w_pad=1)
        
        if scalebar_size: 
            self._add_scalebar_artist(ax2, scalebar_size)
    ...
    def show(self, fig=None, ax=None, colorbar=False, scalebar_size=None):
        ...
        elif (fig is not None) and (ax is None): ax = fig.add_subplot()
        
        im = ax.imshow(self.data, cmap=plt.cm.gist_gray)
        
        if colorbar: plt.colorbar(im, None, ax)
        
        ax.axis('off')

        if scalebar_size: self._add_scalebar_artist(ax, scalebar_size)
...
class FindMaxima():
    ...
    def __init__(self, image, threshold_factor, initial_peaks):
        ...
        self.update(threshold_factor, initial_peaks)

    def update(self, threshold_factor, initial_peaks):
        self.auto_foreground(threshold_factor)
        self.auto_fill_basins(initial_peaks) # see below
...
class PyramidTool():
    ...
    def __init__(self, image, image_name, bool_small_pyramids, threshold_factor, initial_peaks, *args, **kwargs):
        self.image = image
        self.image_name = image_name
        self.bool_small_pyramids = bool_small_pyramids
        self.threshold_factor=threshold_factor
        self.initial_peaks = initial_peaks
        self.find_maxima = FindMaxima(self.image,  self.threshold_factor, self.initial_peaks)
        self.interactive_plot(bool_small_pyramids, *args, **kwargs)


    def interactive_plot(self, bool_small_pyramids, *args, **kwargs):
        fig, axs = plt.subplots(1, 2, sharex=True, sharey=True, *args, **kwargs)
        #fig.suptitle(self.image_name, fontsize=20)
        ...
        ax.set_title(self.image_name, fontsize=16)
        ...
        ax = axs[1]
        self.image.show(fig=fig, ax=ax)
        ax.set_title('Pyramid visualization', fontsize=16)
    ...
    def save_images(self, filename_image):
